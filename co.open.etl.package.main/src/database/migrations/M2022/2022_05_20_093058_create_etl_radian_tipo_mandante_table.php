...
class CreateEtlRadianTipoMandanteTable extends Migration {
    /**
     * Run the migrations.
     *
     * @return void
     */
    public function up() {
        Schema::create('etl_radian_tipo_mandante', function (Blueprint $table) {
            $table->engine = 'InnoDB';

            $table->increments('tma_id')->unsigned()->comment("Id AutoIncremental");
            $table->string('tma_codigo', 20)->comment("Codigo");
            $table->string('tma_descripcion', 100)->comment("Descripcion");
            ...
            $table->index('usuario_creacion','ii1_tma');
            $table->index('tma_codigo', 'ii2_tma');
            $table->foreign('usuario_creacion', 'fk1_etl_radian_tipo_mandante_usuario_creacion')->references('usu_id')->on('auth_usuarios');
        });

        DB::statement("ALTER TABLE etl_radian_tipo_mandante comment 'Tipo Mandante'");
    }
    ...
    public function down() {
        Schema::dropIfExists('etl_radian_tipo_mandante');
    }
}
