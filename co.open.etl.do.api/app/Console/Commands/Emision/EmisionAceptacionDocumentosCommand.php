...
class EmisionAceptacionDocumentosCommand extends Command {
    use DoTrait;
     ...
     * @var string
     */
    protected $signature = 'emision-aceptacion-documentos
     ...
     * @var string
     */
    protected $description = 'Emisión - Procesa la aceptación de documentos';
        ...
        if($agendamiento) {
            ...
            $documentosAgendamiento = EtlEstadosDocumentoDaop::select(['est_id', 'cdo_id', 'est_informacion_adicional'])
                ...
                ->map( function ($estado) use (&$documentosAceptar, &$estadosAceptar, $bdUser) {
                    // Se debe verificar que el documento tenga estados DO y UBLACEPTACION exitosos
                    ...
                    $ublAceptacion = EtlEstadosDocumentoDaop::select(['est_id', 'est_informacion_adicional'])
                        ->where('cdo_id', $estado->cdo_id)
            ...
            if (!empty($documentosAceptar)) {
                ...
                $agendamiento->update([
                    'age_proceso' => 'PROCESANDO-EACEPTACION'
                ]);

                $classAceptar  = new EventStatusUpdate('emision');
                $aceptaciones  = $classAceptar->sendEventStatusUpdate($agendamiento, $user, $documentosAceptar, $estadosAceptar, 'aceptacion');
