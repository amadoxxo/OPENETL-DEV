...
class EmisionAcuseReciboDocumentosCommand extends Command {
    use DoTrait;
     ...
     * @var string
     */
    protected $signature = 'emision-acuse-recibo-documentos
     ...
     * @var string
     */
    protected $description = 'Emisión - Procesa el acuse de recibo de documentos';
    ...
    public function handle() {
        ...
        $agendamiento = AdoAgendamiento::where('age_id', $this->argument('age_id'))
            ->where('age_proceso', 'EACUSERECIBO')
            ->where('estado', 'ACTIVO')
            ->first();
        ...
        if($agendamiento) {
            ...
            $documentosAgendamiento = EtlEstadosDocumentoDaop::select(['est_id', 'cdo_id', 'est_informacion_adicional'])
                ...
                ->map( function ($estado) use (&$documentosAcusar, &$estadosAcusar, $bdUser) {
                    // Se debe verificar que le documento tenga estados DO y UBLACUSERECIBO exitosos
                    ...
                    $ublAcuseRecibo = EtlEstadosDocumentoDaop::select(['est_id', 'est_informacion_adicional'])
                        ->where('cdo_id', $estado->cdo_id)
            ...
            if (!empty($documentosAcusar)) {
                ...
                $agendamiento->update([
                    'age_proceso' => 'PROCESANDO-EACUSERECIBO'
                ]);

                $classAcusar = new EventStatusUpdate('emision');
                $acuses      = $classAcusar->sendEventStatusUpdate($agendamiento, $user, $documentosAcusar, $estadosAcusar, 'acuse');
