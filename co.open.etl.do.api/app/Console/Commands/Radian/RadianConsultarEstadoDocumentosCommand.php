...
class RadianConsultarEstadoDocumentosCommand extends Command {
    /**
     * The name and signature of the console command.
     *
     * @var string
     */
    protected $signature = 'radian-consultar-estado-documentos
     ...
     * @var string
     */
    protected $description = 'Radian - Consulta el estado de los documentos en la DIAN';
    ...
    public function handle(): void {
        ...
        $agendamiento = AdoAgendamiento::where('age_id', $this->argument('age_id'))
            ->where('age_proceso', 'RADGETSTATUS')
            ->where('estado', 'ACTIVO')
            ->first();
        ...
        if($agendamiento) {
            ...
            RadianEstadoDocumentoDaop::select(['est_id', 'cdo_id'])
                ->where('age_id', $this->argument('age_id'))
                ->where('estado', 'ACTIVO')
                ->whereNull('est_resultado')
                ->with([
                    'getRadCabeceraDocumentosDaop:cdo_id,act_id,rfa_prefijo,cdo_consecutivo,cdo_cufe,fecha_creacion',
                ...
                ->map( function ($estado) use (&$documentosConsultar, &$estadosConsultar, $bdUser) {
                    ...
                    $documentosConsultar[$estado->cdo_id] = [
                        'documento'                => $estado->getRadCabeceraDocumentosDaop,
                        ...
                        'ambiente_destino'         => $estado->getRadCabeceraDocumentosDaop->getRadActores->getConfiguracionSoftwareProveedorTecnologico ?
                            $estado->getRadCabeceraDocumentosDaop->getRadActores->getConfiguracionSoftwareProveedorTecnologico->add_id : null,
                        'test_set_id'              => $estado->getRadCabeceraDocumentosDaop->getRadActores->getConfiguracionSoftwareProveedorTecnologico ?
                            $estado->getRadCabeceraDocumentosDaop->getRadActores->getConfiguracionSoftwareProveedorTecnologico->sft_testsetid : null,
            ...
            if (!empty($documentosConsultar)) {
                ...
                $agendamiento->update([
                    'age_proceso' => 'PROCESANDO-RADGETSTATUS'
                ]);

                $classGetStatus = new GetStatus();
                $consultar      = $classGetStatus->consultarDocumentos($agendamiento, $user, $documentosConsultar, $estadosConsultar, true);
