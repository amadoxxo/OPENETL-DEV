...
class RadianAcuseReciboDocumentosCommand extends Command {
     ...
     * @var string
     */
    protected $signature = 'radian-acuse-recibo-documentos
     ...
     * @var string
     */
    protected $description = 'Radian - Procesa el acuse de recibo de documentos';
    ...
    public function handle() {
        ...
        $agendamiento = AdoAgendamiento::where('age_id', $this->argument('age_id'))
            ->where('age_proceso', 'RADACUSERECIBO')
            ->where('estado', 'ACTIVO')
            ->first();
        ...
        if($agendamiento) {
            ...
            RadianEstadoDocumentoDaop::select(['est_id', 'cdo_id', 'est_informacion_adicional'])
                ...
                ->with([
                    'getRadCabeceraDocumentosDaop:cdo_id,act_id,rfa_prefijo,cdo_consecutivo,cdo_cufe,fecha_creacion,cdo_nombre_archivos',
                ...
                ->map( function ($estado) use (&$documentosAcusar, &$estadosAcusar, $bdUser) {
                    ...
                    $ublAcuseRecibo = RadianEstadoDocumentoDaop::select(['est_id', 'est_informacion_adicional'])
                        ->where('cdo_id', $estado->cdo_id)
            ...
            if (!empty($documentosAcusar)) {
                ...
                $agendamiento->update([
                    'age_proceso' => 'PROCESANDO-RADACUSERECIBO'
                ]);

                $classAcusar = new EventStatusUpdate('radian');
                ...
                foreach($acuses as $cdo_id => $resultado) {
                    if($resultado['respuestaProcesada']['estado'] == 'EXITOSO') { 
                        $acuseReciboExitoso = RadianEstadoDocumentoDaop::select(['est_id'])
                            ->where('cdo_id', $cdo_id)
                        ...
                        $acuseFallidos = RadianEstadoDocumentoDaop::select(['est_id'])
                            ->where('cdo_id', $cdo_id)
                        ...
                        if($acuseFallidos < 3) {
                            ...
                            if(!$esXml) {
                                $nuevoAgendamiento = $classAcusar->nuevoAgendamiento('RADACUSERECIBO', $agendamiento, $user, 1);
    ...
    public function agendamientosNotificacionEventos(EventStatusUpdate $classAcusar, User $user, AdoAgendamiento $agendamiento, array $documentosAgendarNotificacionEventos) {
        if(!empty($documentosAgendarNotificacionEventos)) {
            ...
            foreach ($grupos as $grupo) {
                $nuevoAgendamiento = $classAcusar->nuevoAgendamiento('RADUBLADACUSERECIBO', $agendamiento, $user, count($grupo));
