...
use openEtl\Tenant\Models\ProyectosEspeciales\Recepcion\Emssanar\Configuracion\CausalesDevolucion\TenantCausalDevolucion;

class CausalDevolucion extends TenantCausalDevolucion {
    /**
    ...
    public function scopeBusquedaGeneral(Builder $query, string $texto): Builder {
        return $query->where( function ($query) use ($texto) {
            $query->where('cde_descripcion', 'like', '%'.$texto.'%');
        });
    }
    // END SCOPES
}
