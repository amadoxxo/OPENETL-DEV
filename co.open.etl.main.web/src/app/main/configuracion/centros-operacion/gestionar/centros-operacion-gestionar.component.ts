...
@Component({
    selector: 'centros-operacion-gestionar',
    templateUrl: './centros-operacion-gestionar.component.html',
    styleUrls: ['./centros-operacion-gestionar.component.scss']
})
export class CentrosOperacionGestionarComponent extends BaseComponentView implements OnInit{
    ...
    public cop_descripcion: AbstractControl;
    ...
    /**
     * Crea una instancia de CentrosOperacionGestionarComponent.
     * @param {FormBuilder} formBuilder
     * @param {MatDialogRef<CentrosOperacionGestionarComponent>} modalRef
     * @param {any} data
     * @param {ConfiguracionService} _configuracionService
     * @param {JwtHelperService} jwtHelperService
     * @memberof CentrosOperacionGestionarComponent
     */
    constructor(
        private formBuilder: FormBuilder,
        private modalRef: MatDialogRef<CentrosOperacionGestionarComponent>,
    ...
    ngOnInit() {
        if (this.action === 'edit') {
            ...
            this.loadCentroOperacion();   
        } 
        if (this.action === 'view'){
            this.disableFormControl(
                this.cop_descripcion,
    ...
    private initForm(): void {
        this.form = this.formBuilder.group({
            'cop_descripcion': ['', Validators.compose(
                [
                    Validators.required,
        ...
        this.cop_descripcion = this.form.controls['cop_descripcion'];
    ...
    /**
     * Se encarga de cargar los datos de un Centro Operacion que se ha seleccionado en el tracking.
     *
     * @memberof CentrosOperacionGestionarComponent
     */
    public loadCentroOperacion(): void {
        this.loading(true);
        let data: any = {
            cop_descripcion: this.cop_descripcion,
        ...
        this._configuracionService.get(this.IdToGet).subscribe(
            res => {
                if (res) {
                    ...
                    this.cop_descripcion.setValue(res.data.cop_descripcion);
            ...
            error => {
                this.loading(false);
                this.mostrarErrores(error, 'Error al cargar el Centro Operación');
            }
        );
    }
    ...
    /**
     * Crea o actualiza un nuevo Centro Operacion.
     *
     * @param {any} values
     * @memberof CentrosOperacionGestionarComponent
     */
    public saveItem(values: any) {
        ...
        if (this.form.valid) {
            ...
            if (this.action === 'edit') {
                formWithAction.cop_id = this.IdToGet;
                this._configuracionService.update(formWithAction, formWithAction.cop_id).subscribe(
                    response => {
                        this.loading(false);
                        this.showTimerAlert('<strong>Centro Operación actualizado correctamente.</strong>', 'success', 'center', 2000);
                        this.closeModal(true);
                    },
                    error => {
                        this.loading(false);
                        this.mostrarErrores(error, 'Error al actualizar el Centro Operación');
                    });
            } else {
                this._configuracionService.create(formWithAction).subscribe(
                    response => {
                        this.loading(false);
                        this.showTimerAlert('<strong>Centro Operación guardado correctamente.</strong>', 'success', 'center', 2000);
                        this.closeModal(true);
                    },
                    error => {
                        this.loading(false);
                        this.mostrarErrores(error, 'Error al guardar el Centro Operación');
                    });
                }
        }
    }
